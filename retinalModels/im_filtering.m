function im_out= im_filtering(im_in, para)
...
%
%   Inputs: im_in, input image, a matrix
...
[siz_m, siz_n, tmp1, tmp2] =size(im_in); % in case it is a multi-dimensional matrix
...
im_in =im2single(im_in); % convert to single to reduce memory
...
im_out = bsxfun(@times, im_in, D_mat_ring{1});

for m =1:num_inter-1
    switch lower(type_filter)
        case {'gaussian', 'g'}            
        h = fspecial('gaussian', h_siz(m), h_gamma(m));
        im_tmp =imfilter(im_in, h, 'replicate');
